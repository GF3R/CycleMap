...
@Component({
  ...
  standalone: true,
  providers:  [ DataInputService, CoordsManagementService, FeaturesService,  HttpClient ]
})
...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let featureList: Feature[] = this.featuresService.getFeaturesList();
    ...
    if(m <= 5275.9*1000){
      this.map?.getView().fit([Math.min(cordThun[0], point2Coords[0]), Math.min(cordThun[1], point2Coords[1]), 
      Math.max(cordThun[0], point2Coords[0]), Math.max(cordThun[1], point2Coords[1])], 
      {size: this.map.getSize(),
        padding: [200,200,200,200]
      });

    } else {
      this.map?.getView().fit([Math.min(point2Coords[0], cordCapeTown[0]), Math.min(point2Coords[1], cordCapeTown[1]), 
      Math.max(point2Coords[0], cordCapeTown[0]), Math.min(point2Coords[1], cordCapeTown[1])], 
      {size: this.map.getSize(),
        padding: [200,200,200,200] 
    });
        ...
        console.log(this.featuresService.getFeaturesList())
